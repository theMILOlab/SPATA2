...
setMethod(
  ...
  definition = function(object, var_df, vars){
    ...
    var_df <-
      dplyr::select(
        .data = var_df,
        dplyr::all_of(object@coordinates_id),
        dplyr::all_of(vars)
      )

    object@coordinates <-
      dplyr::left_join(x = object@coordinates, y = var_df, by = c("id" = object@coordinates_id))
...
compute_img_scale_fct <- function(hist_img1, hist_img2){
  ...
  base::max(hist_img2@image_info[["dims"]])/
    base::max(hist_img1@image_info[["dims"]][1])
...
createHistoImage <- function(dir,
  ...
  # validate input
  confuns::are_values(c("dir", "img_name", "sample"), mode = "character")
...
#'
#' @inherit argument_dummy params
#' @param ids Character vector. The IDs to exclude.
...
setMethod(
  f = "exclude",
  signature = "HistoImaging",
  definition = function(object, ids, reason){
...
setMethod(
  ...
  definition = function(object, fragments = "all"){
    ...
    if(base::is.character(fragments) &&
      ...
      exclude_ids <- frgmt_df[frgmt_df[["id"]] %in% {{fragments}}][["id"]]
    ...
    object <- exclude(object, ids = exclude_ids, reason = "on_tissue_fragment")
...
#' @inherit ggpLayer_dummy return
#'
#' @details Method for `HistoImaging` and `spata2`.
#'
#' @export
#'
...
setMethod(
  ...
  definition = function(object,
    ...
    } else if(base::is.null(unit)){

      unit <- object@method@unit
...
setMethod(
  ...
  definition = function(object, img_name, ...){
    ...
    dir <- getImageDir(object, img_name = img_name)
      ...
      purrr::map(
        ...
        .f = function(name){
          ...
          if(name == getHistoImageActive(object)@name){
            ...
            obj_plot <- getHistoImage(object, img_name = name)
...
setMethod(
  ...
  definition = function(object, error = FALSE){
    ...
    if(base::isFALSE(out) & base::isTRUE(error)){
      ...
      stop("Tissue sections have not been identified yet.")
